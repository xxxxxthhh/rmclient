"""离线 demo：完整的 rmclient Web，跑在一个内存假云上。全程零网络请求。
    ...
    rmclient demo                # → http://127.0.0.1:8001
    rmclient demo --port 9000
...
本模块随包一起发布：`uvx rmclient demo` 不需要克隆仓库。下面那些最小载荷与
.rm v6 块序列的构造器原先长在 tests/fixtures.py，搬进来之后测试反过来 import
这里——两份实现只留一份。
"""
...
def build_app(cloud: DemoCloud, journal_path: Path):
    ...
    from .web import app, get_client, get_journal
...
def run(port: int = 8001) -> int:
    """起 demo。状态全在内存 + 一个临时目录里，进程一停就没了。"""
    import os
    import tempfile

    import uvicorn

    # demo 自带锁定目录，不看使用者的环境；删除记录也只落到临时目录。
